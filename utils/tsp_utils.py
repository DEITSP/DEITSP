...
def batched_two_opt_torch_cuda(points, tour, max_iterations=1000, device="cpu"):
  iterator = 0
  tour = tour.clone()
  with torch.inference_mode():
    cuda_points = points
...
def merge_tours(adj_mat, np_points, edge_index_np=None, sparse_graph=False, parallel_sampling=1):
  """
...
def greedy_(splitted_real_adj_mat, length):
  tour = [0]
  while len(tour) < length:
    n = np.nonzero(splitted_real_adj_mat[tour[-1]])[0] #nonzero() 得到数组中非零元素的索引
    if len(tour) > 1:
      n = n[n != tour[-2]]
    tour.append(n.max())
  return tour

def merge_tours_mp(adj_mat, np_points, parallel_sampling=1):
  """
  To extract a tour from the inferred adjacency matrix A, we used the following greedy edge insertion
  procedure.
  • Initialize extracted tour with an empty graph with N vertices.
  ...
  # splitted_points = np.split(np_points, parallel_sampling, axis=0)
  splitted_points = [
      np_points for _ in range(parallel_sampling)
  ]
  # S,V,2
  ...
  if parallel_sampling >1:
    length = [splitted_adj_mat[i].shape[0] + 1 for i in range(parallel_sampling)]
    with Pool(parallel_sampling) as p:
      tours = p.starmap(
          greedy_,
          zip(splitted_real_adj_mat, length),
      )
  else:
    tours = []
    for i in range(parallel_sampling):
      tour = [0]
      while len(tour) < splitted_adj_mat[i].shape[0] + 1:
        n = np.nonzero(splitted_real_adj_mat[i][tour[-1]])[0] #nonzero() 得到数组中非零元素的索引
        if len(tour) > 1:
          n = n[n != tour[-2]]
        tour.append(n.max())
      tours.append(tour)
...
def tsp_evaluate(dist_mat, route):
  total_cost = 0
  for i in range(len(route) - 1):
    total_cost += dist_mat[route[i], route[i + 1]]
  return total_cost

def tsp_evaluate_mp(dist_mat, routes, num_mp):
  with Pool(num_mp) as p:
    total_costs = p.starmap(
        tsp_evaluate,
        zip([dist_mat]*num_mp,routes)
    )
  return total_costs
  
class TSPEvaluator_mp(object):
  def __init__(self, points):
    self.dist_mat = scipy.spatial.distance_matrix(points, points)
  ...
  def evaluate_mp(self, routes, num_mp):
    with Pool(num_mp) as p:
      total_costs = p.starmap(
          self.evaluate,
          routes,
